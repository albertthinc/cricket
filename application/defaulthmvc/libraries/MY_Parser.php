...
class MY_Parser extends CI_Parser {
    ...
    */
    public function __call($method, $params=array())
    {
        if (! method_exists($this, $method) ) {
            return call_user_func_array(array($this->CI->smarty, $method), 
                    $params);
        }
    }
    ...
    public function set_theme($name)
    {
        ...
        $functions_file = config_item('theme_path') . $this->_theme_name 
                . '/functions.php';

        // Incase we have a theme in the application directory
        $functions_file2 = APPPATH."themes/" . $this->_theme_name 
                . '/functions.php';
            ...
            $caching = FALSE, $theme = '')
    {
        ...
        // If no file extension dot has been found default to defined extension 
        // for view extensions
        if (! stripos($template, '.')) {
        ...
        // If we're returning the templates contents, we're displaying the 
        // template
        if ($return === FALSE) {
    ...
    public function css($file, $attributes = array())
    {
        ...
        $return = '<link rel="' . $attributes['rel'] . '" type="' 
                . $attributes['type'] . '" href="' 
                . base_url(config_item('theme_path') . $this->get_theme() 
                        . "/css/" . $file) 
    ...
    public function img($file, $attributes = array())
    {
        ...
        $return = '<img src ="' . base_url(config_item('theme_path') 
                . $this->get_theme() . "/img/" . $file) . '" alt="' 
                . $attributes['alt'] . '" title="' . $attributes['title'] 
                . '" />';
    ...
    */
    ...
    protected function _update_theme_paths()
    {
        ...
        $this->_template_locations = array(
            config_item('theme_path') . $this->_theme_name . '/views/modules/' 
            . $this->_module . '/layouts/',
            config_item('theme_path') . $this->_theme_name . '/views/modules/' 
            . $this->_module .'/',
    ...
    */
    public function string_parse($template, $data = array(), $return = FALSE, 
            $is_include = FALSE)
    {
